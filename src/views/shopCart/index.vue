...
import { getAddressDataApi, setDefaultAddressApi } from "@/api/address"
import { useUserStore } from "@/store/modules/user"
import { ElMessage, ElMessageBox } from "element-plus"
import { createSellPurchaseDataApi } from "@/api/sellPurchase"
import { GetTableData } from "@/api/table/types/table"
import { GetAddressData } from "@/api/address/types/Address"
...
const addressData = ref<GetAddressData>({})
const addressDataAll = ref<GetAddressData[]>([])
...
const deleteShopCarts = () => {
  loading.value = true
  deleteShoppingCartDataApi(shopCartIds.value)
    .then(() => {
...
const handleDelete = (row: GetTableData) => {
  ...
  ElMessageBox.confirm(`正在删除商品：${row.title}，确认删除？`, "提示", {
    ...
    deleteShoppingCartDataApi([row.shoppingId])
      .then(() => {
        ElMessage.success("删除成功")
        getShopCartData()
      })
...
const submitChange = () => {
  ...
  setDefaultAddressApi({ defaultAddress: defaultAddress.value, newAddress: newAddress.value })
    .then(() => {
...
sums.value = computed(() => {
  let sums = 0
  shopCartData.value.forEach((item) => {
...
const payment = () => {
  ElMessageBox.confirm("确认支付吗", "提示", {
    ...
    .then(() => {
      // getPayData()
      createSellPurchaseDataApi(getPayData()).then(() => {
        getShopCartData()
      ...
      <div class="receiving-address">
        ...
        <el-dialog title="更换地址" v-model="dialogVisible" width="580px">
          ...
          >
            ...
            <el-table-column label="收货人" prop="consignee" width="120" show-overflow-tooltip />
            <el-table-column prop="addressDetail" label="收获地址" show-overflow-tooltip />
            <el-table-column prop="phone" label="收获手机号" width="120" show-overflow-tooltip />
          </el-table>
      ...
      <el-table :data="shopCartData" @selection-change="handleShopSelectionChange">
        ...
        <el-table-column prop="good" label="商品" width="500">
          <template #default="scope">
            <el-row :gutter="20">
              <el-col :span="5">
                <img style="width: 100px" :src="scope.row.picture" class="image" />
              </el-col>
