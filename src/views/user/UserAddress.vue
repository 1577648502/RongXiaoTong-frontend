<template>
  <div class="add-address">
    ...
    <div v-for="(item, index) in allAddressData" :key="index" class="default-address-container">
      ...
      <div class="right-btn">
        <el-tag
          style="position: relative; top: -40px; right: -138px; margin-right: 20px"
    ...
    <el-dialog title="更换地址" v-model:visible="dialogVisible" width="580px" :before-close="handleClose">
      <el-table
        ref="multipleTable"
        :data="allAddressData"
        tooltip-effect="dark"
    ...
    <el-dialog :title="title" v-model="showAdd" width="580px" :before-close="closeAdd">
      <el-form :model="addressData" ref="ruleForm" label-width="100px" class="demo-ruleForm">
        <el-form-item label="收货人：" prop="consignee">
          <el-input v-model="addressData.consignee" />
        </el-form-item>
        <el-form-item label="收货地址：" prop="addressDetail">
          <el-input v-model="addressData.addressDetail" />
        </el-form-item>
        <el-form-item label="收货手机号：" prop="phone">
          <el-input v-model="addressData.phone" />
        </el-form-item>
        <el-checkbox true-label="1" false-label="0" style="float: right" v-model="addressData.isDefault"
          >设为默认地址
        </el-checkbox>
      </el-form>
...
const allAddressData = ref<GetAddressData[]>([])
...
const getAllAdress = () => {
  loading.value = true
  getAddressDataApi({ ownName: ownName })
    .then((res) => {
      loading.value = false
